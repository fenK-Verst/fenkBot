...
const
    {SessionManager} = require('@vk-io/session'),
    {SceneManager, StepScene} = require('@vk-io/scenes'),
...
let day: string | number | undefined,
    month: string | number,
    year: any
;
...
function getRasp(date: Date) {
    return new Promise((resolve, reject) => {
        if (isNaN(date.getTime())) {
            ...
            // @ts-ignore
            date = date.getSqlite();
            db.all('SELECT l1,l2,l3,l4,l5,l6,dz, date FROM lessons WHERE date=?', date, function (err: any, rows: any) {
                if (err) {
                    console.log(err.message);
                    reject(err.message)
                }
...
function inInterval(value: any, p: string) {
