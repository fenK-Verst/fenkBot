...
const fs = require('fs'), data = fs.readFileSync('data.json', 'utf-8'), TOKEN = JSON.parse(data).token, vk = new vk_io_1.VK({ token: TOKEN }), { api } = vk, sqlite3 = require('sqlite3').verbose(), db = new sqlite3.Database('schedule.db');
let rasp = 'https://sun9-3.userapi.com/c846122/v846122093/d6776/NqM_20zkAbM.jpg';
const { SessionManager } = require('@vk-io/session'), { SceneManager, StepScene } = require('@vk-io/scenes'), sessionManager = new SessionManager(), sceneManager = new SceneManager();
// @ts-ignore
...
// vk.updates.hear('/start', async (context: any) => {
//     await context.send(`
// 		My commands list
// 		/cat - Фото котика
// 		/time - Показать дату
...
//     } else {
//         db.all('SELECT l' + para + ' FROM lessons WHERE date=?', dat, function (err: any, rows: any) {
//             if (err) {
...
// });
//
// vk.updates.hear(/^\/reverse (.+)/i, async (context: any) => {
//     await context.send(
...
// });
//
// vk.updates.hear(/^\/ras/i, async (context: any) => {
//
...
// });
//
// vk.updates.hear(/^\/about/i, async (context: any) => {
//     context.send("Bot for schedule 17-VT-1 by @id161830362 (fenK) ")
//
// });
// vk.updates.hear(/^\/imp/i, async (context: any) => {
//     let oFile = fs.readFileSync('oFile.txt', 'utf-8');
//     context.send(oFile);
//
// });
// vk.updates.hear(/^\/simp/i, async (context: any) => {
//     await context.scene.enter('OVS');
// });
//
// vk.updates.hear(/^\/kek/i, async (context: any) => {
//
...
// vk.updates.hear(/^\/upr/i, async (context: any) => {
...
//
// });
// vk.updates.hear(/^\/dz/i, async (context: any) => {
//     if ([161830362, 259251175, 503131193, 262742265].includes(context.senderId)) {
...
//
// });
// vk.updates.hear("/ex", async (context: any) => {
...
//
//     fetch(link)
//         .then((res: { buffer: () => Promise<string | any[]>; }) => {
//
//             res.buffer().then((b: string | any[]) => {
//                 fs.open(`rasp.xlsx`, 'w', function (err: any, fd: any) {
//                     if (err) {
//                         throw 'could not open file: ' + err;
//                     }
//
//                     // write the contents of the buffer, from position 0 to the end, to the file descriptor returned in opening our file
//                     fs.write(fd, b, 0, b.length, null, function (err: any) {
...
//
//                                             date = date.getSqlite();
//                                             db.run(`INSERT INTO lessons(date,l1,l2,l3,l4,l5,l6)VALUES (?,?,?,?,?,?,?)`,
//                                                 [date, l1, l2, l3, l4, l5, l6],
